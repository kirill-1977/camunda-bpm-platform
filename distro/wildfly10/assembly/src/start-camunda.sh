...
if [ -z "$BROWSER" ]; then
  ...
  (sleep 15; $BROWSER "http://localhost:8180/camunda-welcome/index.html";) &
fi

/bin/sh "$(dirname "$0")/server/wildfly-${version.wildfly10}/bin/standalone.sh" -Djboss.http.port=8180
